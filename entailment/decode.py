...
def decode(model: PreTrainedModel,
           input_ids: Tensor,
           tokenizer: PreTrainedTokenizer,
           source_text: str,
           max_length: int,
           prepend_decoded_token_ids: Optional[List[int]] = None,
    ...
    # TODO: maybe add this as an argument to the function?
    ...
    noun_toks = get_noun_bpe_tok_ids_in_source_text(source_text=source_text,
                                                    tokenizer=tokenizer)


    if model.config.is_encoder_decoder:
        # create empty decoder_input_ids
        cur_len = len(prepend_decoded_token_ids) + 1 if prepend_decoded_token_ids else 1
...
def _generate_beam_search(
    ...
    while cur_len < max_length:
        ...
        if do_sample:
            ...
            # _scores = noun_token_reweighting(logits=_scores,
            #                                  noun_ids=noun_toks_ids)
...
def get_noun_bpe_tok_ids_in_source_text(source_text: str,
                                        tokenizer: PreTrainedTokenizer) -> List[List[int]]:
    valid_pos_tags_to_keep = {"PROPN", "NUM", "NOUN"}
    doc = nlp(source_text)
    nouns_tok_ids = []
    for tok in doc:
        if tok.pos_ in valid_pos_tags_to_keep:
            tok_ids = tokenizer.encode(tok.text, add_special_tokens=False)
            print(tok.text, tok_ids)
            nouns_tok_ids.append(tok_ids)

    return nouns_tok_ids


def noun_token_reweighting(logits: Tensor,
                           noun_ids: List[List[int]],
                           top_k: int = 8,
                           noun_ratio_th: float = 0.05,
                           filter_offset: float = 50):
    batch_inds = torch.topk(logits, top_k)[1]
    batch_inds = batch_inds.detach().cpu().numpy().tolist()

    noun_ids = [_ids[0] for _ids in noun_ids]

    noun_count_th = min(1, int(len(noun_ids) * noun_ratio_th))
    noun_ids_tensor = torch.tensor(noun_ids)

    for row_idx, inds in enumerate(batch_inds):
        _intersect = [id for id in inds if id in noun_ids]
        if len(_intersect) >= noun_count_th:
            _mask = torch.zeros_like(logits[row_idx])
            _mask[noun_ids_tensor] = 1
            logits[row_idx] += _mask * filter_offset
